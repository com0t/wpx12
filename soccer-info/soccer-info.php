...
if ( !class_exists('SoccerInfo') ) {
	...
	class SoccerInfo {
		...
		public function __construct() {
			...
			define('SOCCER_INFO_VERSION', '1.9.8');
		...
		function wpsi_results($filtered_html, $highlight = '', $team = '', $limit = 0, $team_id = 0, $enetpulse = false, $icon = 0, $link = 1, $nofollow = 0) {
			...
			if ( isset($table) && is_object($table) ) {
				$f_rand = rand();
				$filtered_html = '<table id="sitafire'.$f_rand.'">'."\n";
					...
					foreach ( $table_tbody_0 as $ii => $tr ) {
						...
						if ( $limit == 0 || $i_limit < $limit ) {
							...
							if ( ( count($tr->td) > 4 + $van_comp && isset($tr->td[0]->span[0] ) || 
								 ( $enetpulse != false && isset($tr->div[0]->div) && count($tr->div[0]->div) > 4 + $van_comp && strpos($tr['class'], 'finished') !== false ) ) ) {
								//echo "szilva";
								if ( $enetpulse != false ) {
				...
				if ( is_object($table->tbody) ) {
					...
					foreach ( $table_tbody_0 as $ii => $tr ) {
						...
						if ( $limit == 0 || $i_limit < $limit ) {
							...
							if ( ( count($tr->td) > 4 + $van_comp && isset($tr->td[0]->span[0]) ) || 
								 ( $enetpulse != false && isset($tr->div[0]->div) && count($tr->div[0]->div) > 4 + $van_comp && strpos($tr->div[0]->div[0]->div[0]->div[0]->span[0], '/') !== false ) ) {
